...
debug = dt.timedelta(hours=0)

# current_time_UTC  = dt.datetime.now(dt.timezone.utc).replace(tzinfo=None) + debug
current_time_local= dt.datetime.now() + debug
...
print(f"Local time: {current_time_local.strftime("%H:%M:%S.%f")[:-4]}")
...
line1 = current_time_local.strftime("%H:%M:%S.%f")[:-4] # calling it this way ensures it's not in UTC but in correct time zone.
